...
from infrastructure.db.models import AIUsageLogModel, AIDailyLimitModel, AIResponseCacheModel
...
class AIUsageTracker:
    ...
    Almacena en tablas PostgreSQL (SQLAlchemy):
    - ai_usage_logs: logs de uso individuales
    - ai_daily_limits: contadores diarios por usuario
    - ai_response_cache: cache de respuestas por hash de prompt

    Forma de un log de uso:
    {
        'id': str,
        'usuario': str,
    ...
    def check_user_limits(usuario: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            db = SessionLocal()
            try:
                limit = db.get(AIDailyLimitModel, limit_doc_id)

                if not limit:
                    return (True, None)

                requests_today = limit.requests_count or 0
                tokens_today = limit.tokens_count or 0
    ...
    def get_cached_response(prompt: str, model: str = "gemini-pro") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                row = (
                    db.query(AIResponseCacheModel)
                    .filter(
                        AIResponseCacheModel.prompt_hash == prompt_hash,
                        AIResponseCacheModel.model == model,
                        AIResponseCacheModel.timestamp >= yesterday
                    )
    ...
    def log_usage(
        ...
        """
        Registra el uso de IA en la tabla ai_usage_logs y actualiza contadores diarios
            ...
            try:
                ...
                if success:
                    ...
                    if limit is None:
                        ...
                        db.add(limit)
                    limit.requests_count = (limit.requests_count or 0) + 1
                    limit.tokens_count = (limit.tokens_count or 0) + total_tokens
                    limit.total_cost_usd = (limit.total_cost_usd or 0.0) + estimated_cost
                    limit.last_updated = datetime.now().isoformat()
    ...
    def get_user_usage_today(usuario: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                requests = limit.requests_count or 0
                tokens = limit.tokens_count or 0

                return {
                    'requests_count': requests,
                    'tokens_count': tokens,
                    'total_cost_usd': limit.total_cost_usd or 0.0,
